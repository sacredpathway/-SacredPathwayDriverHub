...
struct ManualPaystubView: View {
    ...
    // the instant a Driver/Owner Operator/Carrier purchase or restore lifts the entitlement.
    ...
    private func generateManualPDF() {
        ...
        Task { @MainActor in
            do {
                ...
                pdfData = data
                SPHaptics.success()   // paystub rendered
                showingPreview = true

                // Review-prompt milestone — same policy as the settlement
                // generator; service gates frequency and eligibility.
                ...
                //
                // Stability fix (Phase 1 · Task 5): failure is surfaced —
                // silently-unsettled loads reappear in future pickers and
                // risk double-payment.
                ...
                if !importedIds.isEmpty {
                    Task { @MainActor in
                        do {
                            ...
                            errorMessage = "Paystub created, but the imported loads couldn't be marked as settled (network issue). They may show up in the next paystub picker — settle them manually to avoid double-paying."
                            #if DEBUG
                            print("[ManualPaystubView] markLoadsAsSettled failed: \(error)")
                            #endif
                        }
                    }
                }
