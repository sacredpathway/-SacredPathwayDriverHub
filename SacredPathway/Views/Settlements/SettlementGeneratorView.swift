...
struct SettlementGeneratorView: View {
    ...
    // immediately after a Driver/Owner Operator/Carrier purchase or restore.
    ...
    /// S9: fingerprint of the SELECTIONS (loads + driver + period) behind
    /// the current `calculation`. When live selections drift from it, the
    /// on-screen preview is STALE — the user could otherwise export a PDF
    /// whose numbers they never saw (generatePDF always recalcs with the
    /// CURRENT selections). Stale ⇒ preview dims + export hides until
    /// recalculated. Set in exactly one place (end of runMatchAndCalc) so
    /// every calc path — Calculate, pull-refresh, expense-change, export
    /// pre-render — keeps it honest.
    @State private var lastCalcSelectionFingerprint: String = ""
    ...
    // MARK: - S9 live-preview integrity

    /// Deterministic fingerprint of everything that changes the settlement
    /// besides expenses (those have their own fingerprint): selected load
    /// ids, driver, and period bounds (day precision — matches the picker).
    private var selectionFingerprint: String {
        let ids = selectedLoadIds.map(\.uuidString).sorted().joined(separator: "|")
        let driver = selectedDriver?.id?.uuidString ?? "owner-op"
        let f = DateFormatter()
        f.locale = Locale(identifier: "en_US_POSIX")
        f.dateFormat = "yyyyMMdd"
        return "\(ids)#\(driver)#\(f.string(from: periodStart))-\(f.string(from: periodEnd))"
    }

    /// True when the on-screen calculation no longer matches the current
    /// selections. Gates the export buttons (no exporting unseen numbers).
    private var calculationIsStale: Bool {
        calculation != nil && selectionFingerprint != lastCalcSelectionFingerprint
    }

    /// Instant, always-live gross for the current selection — a pure sum of
    /// the picked loads' revenue. Deliberately NOT a fee estimate: partial
    /// fee math without the matched expenses would show a wrong "net" and
    /// that's exactly the kind of silent drift this session forbids.
    private var selectedGross: Double {
        selectedLoads.reduce(0) { $0 + ($1.totalRevenue ?? 0) }
    }
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // S9: instant selection strip — live gross for what's
                    // picked, before any Calculate tap. Pure sum, no fee
                    // estimate (partial math would mislead).
                    if !selectedLoadIds.isEmpty {
                        HStack(spacing: 6) {
                            Image(systemName: "sum")
                                .font(.caption)
                                .accessibilityHidden(true)
                            Text("\(selectedLoadIds.count) load\(selectedLoadIds.count == 1 ? "" : "s") selected · \(selectedGross.asCurrency) gross")
                                .font(.caption.weight(.semibold))
                                .contentTransition(.numericText())
                                .animation(.spQuick, value: selectedGross)
                        }
                    ...
                    if let calc = calculation {
                        ...
                        settlementPreview(calc)
                            .opacity(calculationIsStale ? 0.45 : 1)
                        // Stale numbers can't be exported — the user must
                        // recalculate so what they SEE is what they SIGN.
                        if !calculationIsStale {
                            exportButton
                            settleConsequenceNote
                        }
                    }
    ...
    private var calculateButton: some View {
        Button {
            ...
            HStack {
                ...
                Text(selectedLoadIds.isEmpty
                     ? "Pick loads to calculate"
                     : (calculationIsStale
                        ? "Recalculate Settlement (\(selectedLoadIds.count))"
                        : "Calculate Settlement (\(selectedLoadIds.count))"))
            }
            .font(.headline)
    ...
    /// S9: shown between a stale preview and the Calculate button — the
    /// selections changed underneath the numbers on screen.
    private var staleCalculationBanner: some View {
        HStack(spacing: 8) {
            Image(systemName: "exclamationmark.arrow.circlepath")
                .foregroundStyle(Color.spWarning)
                .accessibilityHidden(true)
            Text("Selections changed since this preview was calculated. Recalculate to refresh the numbers — export is disabled until then.")
                .font(.caption)
                .foregroundStyle(Color.spTextSecondary)
    ...
    /// S9: explicit consequence line under the export buttons — exporting
    /// is what flips loads to `settled`, and the user should know before
    /// tapping, not after.
    private var settleConsequenceNote: some View {
        Text("Exporting marks the \(selectedLoadIds.count) selected load\(selectedLoadIds.count == 1 ? "" : "s") as settled — they'll leave future paystub pickers unless you show settled loads.")
            .font(.caption2)
            .foregroundStyle(Color.spTextSecondary)
            .frame(maxWidth: .infinity, alignment: .leading)
            .fixedSize(horizontal: false, vertical: true)
    ...
    private func calcRow(_ label: String, _ value: String, bold: Bool = false, color: Color = .spTextPrimary) -> some View {
        ...
        // One VoiceOver stop per line item ("Factoring Fee, minus $73.50").
        .accessibilityElement(children: .combine)
    }
    ...
    private func runMatchAndCalc(chosenLoads: [Load]) async throws {
        ...
        // S9: single sync point for selection freshness — every calc path
        // (Calculate, pull-refresh, expense-change, export pre-render)
        // funnels through here, so a completed calc always clears staleness
        // for the selections it actually used.
        lastCalcSelectionFingerprint = selectionFingerprint
    }
    ...
    private func generatePDF(showPreview: Bool = true) async {
        ...
        do {
            ...
            pdfData = data
            SPHaptics.success()   // paystub rendered — the app's money moment
            if showPreview { showingPreview = true } else { showingShareSheet = true }

            // Review-prompt milestone: a successfully generated paystub is a
            // moment of delivered value. All gating (3rd success, 30-day
            // spacing, once per version) lives inside the service.
            ...
            //
            // Stability fix (Phase 1 · Task 5): this used to be `try?` —
            // a silent failure left the loads UNSETTLED, so they reappeared
            // in the next paystub picker and could be paid twice. The PDF
            // is already generated at this point, so a failure must not
            // block sharing — but the user MUST be told.
            let ids = selectedLoads.compactMap { $0.id }
            do {
                try await supabase.markLoadsAsSettled(loadIds: ids)
            } catch {
                errorMessage = "Paystub created, but the loads couldn't be marked as settled (network issue). They may show up in the next paystub picker — pull to refresh and settle them manually to avoid double-paying."
                #if DEBUG
                print("[SettlementGeneratorView] markLoadsAsSettled failed: \(error)")
                #endif
            }
