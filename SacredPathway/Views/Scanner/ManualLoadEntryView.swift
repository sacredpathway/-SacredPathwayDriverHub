...
struct ManualLoadEntryView: View {
    ...
    // Load date — defaults to today so a new load immediately counts in the
    // current week/month on the dashboard. Editable here and in load detail.
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        // Load Date — drives which week/month the load counts in.
                        sectionCard("Load Date") {
                            DatePicker(
                                "Load Date",
    ...
    private func prefillIfNeeded() {
        ...
        // Preserve the existing load date when editing; default to today otherwise.
        ...
        if AppMode.shared.isLocal {
            ...
            if let existing = existingLoad, existing.id != nil {
                ...
                let load = Load(
                    ...
                    // Stamp the load date to TODAY so the entry immediately counts
                    // in This Week / This Month / All-Time on the dashboard. (User
                    // feedback: new loads weren't showing in the weekly/monthly
                    // totals because manual entries had no date.) Editable later in
                    ...
                    // "active" instead of "pending": a newly added load counts as a
                    // normal completed/active load right away (no pending hold).
                    status: "active"
                )
