...
struct DocumentReviewView: View {
        ...
        VStack(spacing: 20) {
            ...
            VStack(spacing: 12) {
                ...
                Button {
                    ...
                    UIPasteboard.general.string = debugText.isEmpty ? rawExtractionText : debugText
    ...
    /// Parse a rate-con date string from AI extraction into a `Date`. Tries the
    /// canonical Supabase/ISO formats first (via SPDate), then the common US
    /// rate-con formats (MM/dd/yyyy, M/d/yy, "Apr 19, 2026", …). Returns nil if
    /// unparseable, so the load falls back to createdAt for week bucketing.
    private func parseRateConDate(_ s: String?) -> Date? {
        guard let raw = s?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty else { return nil }
        if let d = SPDate.parse(raw) { return d }
        let f = DateFormatter()
        f.locale = Locale(identifier: "en_US_POSIX")
        f.timeZone = TimeZone(secondsFromGMT: 0)
        for fmt in ["MM/dd/yyyy", "M/d/yyyy", "MM-dd-yyyy", "M/d/yy", "MM/dd/yy",
                    "yyyy/MM/dd", "MMM d, yyyy", "MMMM d, yyyy", "MMM d yyyy"] {
            f.dateFormat = fmt
            ...
            do {
                ...
                if isExpenseType {
                    ...
                    let load = Load(
                        ...
                        loadNumber: loadNumber.isEmpty ? nil : loadNumber,
                        brokerName: brokerName.isEmpty ? nil : brokerName,
                        // Rate-con dates parsed from the document. These drive
                        // weekly/monthly bucketing (pickup first, then delivery)
                        // so a scanned load counts in the week shown on the rate
                        // con — NOT the day it was scanned/imported. Falls back to
                        // createdAt only when the rate con has no dates.
                        pickupDate: parseRateConDate(extractedData?.pickupDate),
                        deliveryDate: parseRateConDate(extractedData?.deliveryDate),
                        origin: origin.isEmpty ? nil : origin,
                        destination: destination.isEmpty ? nil : destination,
                        totalMiles: Double(totalMiles),
                        lineHaulRate: Double(lineHaulRate),
                        ...
                        accessorialCharges: Double(accessorialCharges),
                        totalRevenue: Double(totalRevenue),
                        status: "active"   // new loads default to Active, never Pending
                    )
