...
struct SmartScanReviewView: View {
    ...
    @State private var copiedOCRDebugText = false
    ...
    var body: some View {
        if true {
            NavigationStack {
                ...
                .onAppear(perform: handleReviewAppear)
    ...
    private var rawOCRDisclosure: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if showRawOCR {
                #if DEBUG
                Button {
                    let debugText = LocalDocumentParser.SmartScanDebugStore.lastText
                    UIPasteboard.general.string = debugText.isEmpty ? parsed.rawText : debugText
                    copiedOCRDebugText = true
                } label: {
                    Label(copiedOCRDebugText ? "Copied OCR Debug Text" : "Copy OCR Debug Text",
                          systemImage: copiedOCRDebugText ? "checkmark.circle.fill" : "doc.on.doc.fill")
                        .font(.caption.weight(.semibold))
    ...
    private func handleReviewAppear() {
        prefillFromParsed()
        DispatchQueue.main.async {
            applyEquipmentDefaultsIfNeeded()
            logReviewParsedObject(stage: "appear-async")
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.75) {
            applyEquipmentDefaultsIfNeeded()
            logReviewParsedObject(stage: "appear-delayed")
        }
    }
    ...
    private func prefillFromParsed() {
        ...
        brokerEmail          = sanitizedBrokerEmail(parsed.brokerEmail ?? "")
        brokerMcNumber       = parsed.brokerMcNumber       ?? ""
        #if DEBUG
        print("[SP_DEBUG_RATECON] prefill brokerName='\(brokerName)' MC from OCR='\(brokerMcNumber)'")
        if let parsedEmail = parsed.brokerEmail,
           !parsedEmail.isEmpty,
           brokerEmail.isEmpty {
            print("[DriverHub Parser] rejected owner/company email: \(parsedEmail)")
        }
        #endif
        ...
        // Keep PO independent from Load #. If OCR did not find an explicit PO
        // label, blank is safer than mirroring a questionable identifier.
        ...
        logReviewParsedObject(stage: "prefill")
    }
    ...
    private func applyEquipmentDefaultsIfNeeded(allowParsedTrailer: Bool = false) {
        ...
        #if DEBUG
        print("[SP_DEBUG_RATECON] equipment defaults allowParsedTrailer=\(allowParsedTrailer) currentTruck='\(truckNumber)' currentTrailer='\(trailerNumber)' \(DriverEquipmentProfileStore.debugEquipmentSnapshot(profile: supabase.currentProfile))")
        #endif
        ...
        if allowParsedTrailer,
           ...
           isPlausibleTrailerNumber(parsedTrailer) {
    ...
    private func logReviewParsedObject(stage: String) {
        #if DEBUG
        print("""
        [SP_DEBUG_RATECON] REVIEW_OBJECT stage=\(stage)
          load=\(loadNumber.isEmpty ? "<blank>" : loadNumber)
          broker=\(brokerName.isEmpty ? "<blank>" : brokerName)
          weight=\(weight.isEmpty ? "<blank>" : weight)
          pickupAddress=\(pickupAddress.isEmpty ? "<blank>" : pickupAddress)
          deliveryAddress=\(deliveryAddress.isEmpty ? "<blank>" : deliveryAddress)
          truck=\(truckNumber.isEmpty ? "<blank>" : truckNumber)
          trailer=\(trailerNumber.isEmpty ? "<blank>" : trailerNumber)
          bol=\(bolNumber.isEmpty ? "<blank>" : bolNumber)
          po=\(poNumber.isEmpty ? "<blank>" : poNumber)
        """)
        #endif
    }

    private func isPlausibleTrailerNumber(_ value: String) -> Bool {
        ...
        let blocked = ["van", "reefer", "flatbed", "dat", "dry van", "equipment"]
        return !blocked.contains { lower.contains($0) }
    }

    private func sanitizedBrokerEmail(_ value: String) -> String {
        ...
        return blocked.contains { email.contains($0) } ? "" : email
    }
    ...
    private func updateMatchedBroker() {
        ...
        if let m = matchedBroker {
            if brokerMcNumber.isEmpty, let mc = m.mcNumber {
                ...
                print("[DriverHub Parser] selected broker MC: \(mc)")
                #endif
            } else {
                #if DEBUG
                if brokerMcNumber.isEmpty {
                    print("[SP_DEBUG_RATECON] saved broker matched but has no MC on file (brokerName='\(brokerName)')")
                    print("[DriverHub Parser] selected broker MC: <nil> (enrichment matched broker but no MC on file)")
                }
                #endif
            }
        } else {
            ...
            if brokerMcNumber.isEmpty {
                ...
                print("[DriverHub Parser] selected broker MC: <nil> (low-confidence enrichment/no saved broker match)")
            }
            #endif
        }
    }
    ...
    private func autoAddBrokerContactLocal(profileId: UUID) async -> (brokerId: UUID?, contactId: UUID?) {
        ...
        let emailToSave = sanitizedBrokerEmail(brokerEmail)
    ...
    private func patchLoadWithBrokerAttributionLocal(
        ...
        let email = sanitizedBrokerEmail(brokerEmail)
    ...
    private func patchLoadWithBrokerAttribution(
        ...
        patched.brokerContactEmail   = email.isEmpty ? nil : email
    ...
    private func addBrokerToContactsAndFinish() async {
        ...
        do {
            ...
            if let id = created.id,
               !(brokerPhone.isEmpty && emailToSave.isEmpty && extTrimmed.isEmpty) {
                let contact = BrokerContact(
                    brokerId: id,
