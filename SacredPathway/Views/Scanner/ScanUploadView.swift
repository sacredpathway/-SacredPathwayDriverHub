...
enum FeatureFlags {
    ...
    // Driver Hub boundary: only the driver-side Dispatcher Connect surface
    // is allowed here. Dispatcher dashboards, directories, load-board tools,
    // invoices, and payment workflows belong in the future separate
    // Dispatcher App.
    static let dispatcherConnectEnabled = true
}
...
struct ScanUploadView: View {
    ...
    var body: some View {
            ...
            NavigationStack {
                VStack(spacing: 16) {
                    ...
                    if FeatureFlags.smartScanEnabled {
                        ...
                        .buttonStyle(.plain)
                        .disabled(isParsing)
                        // Receipt scanning lives in the Expenses tab now — a
                        // scanned receipt creates an EXPENSE, not a load.
                    }
        ...
        .sheet(isPresented: $showPhotoPicker) {
            PhotoPickerView(
                onPick: { image in
                    handleSmartScanImages([image], previewImage: image, originalData: nil, mime: nil)
                },
                onCancel: { }
            )
        }
    ...
    private func handleSmartScanImages(_ images: [UIImage],
                                       previewImage: UIImage,
                                       originalData: Data?,
                                       mime: String?) {
        scannedImage = previewImage
