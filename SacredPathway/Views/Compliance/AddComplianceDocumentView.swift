...
struct AddComplianceDocumentView: View {
    ...
    private var attachmentSection: some View {
        let cardBackground = Color.spCardBg
        let primaryText = Color.spTextPrimary
    ...
    private func saveDocument() async {
        ...
        do {
            ...
            let saved = try await supabase.createComplianceDocument(doc)
            if let id = saved.id {
                await ComplianceReminderService.shared.reconcile(
                    documentID: id,
