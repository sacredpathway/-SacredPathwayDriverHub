...
//    • Higher tiers inherit every feature from lower tiers.
//
...
enum SubscriptionTier: String, CaseIterable, Identifiable, Comparable {
    ...
    var monthlyPriceText: String? {
        ...
        case .driver:        return "$14.99/month"
        case .ownerOperator: return "$29.99/month"
        case .carrier:       return "$49.99/month"
        }
    }
...
final class SubscriptionService: ObservableObject {
    ...
    typealias Feature = FeatureAccessManager.Feature
    ...
    func refreshEntitlements() async {
        ...
        if DemoMode.unlockAllFeatures {
            self.activeProductIDs = [Config.IAP.carrierMonthly]
        ...
        self.activeProductIDs = activeIDs
    ...
    func tier(for productID: String) -> SubscriptionTier {
        FeatureAccessManager.tier(for: productID)
    }

    /// Products that belong to a given tier — used by the paywall to split
    /// the product list into Driver, Owner Operator, and Carrier plans.
    func products(for tier: SubscriptionTier) -> [Product] {
        products.filter { self.tier(for: $0.id) == tier }
    }

    /// Driver Hub app mode derived from the active StoreKit entitlement.
    /// Profile `account_role` is intentionally not consulted here; old profile
    /// rows can no longer switch the app into a different dashboard.
    var entitledAccountRole: AccountRole? {
        #if DEBUG
        if DemoMode.unlockAllFeatures { return .carrier }
        #endif

        if !FeatureFlags.subscriptionsEnabled { return .ownerOperator }

        if activeProductIDs.contains(Config.IAP.carrierMonthly) ||
            activeProductIDs.contains(Config.IAP.carrierAnnual) {
            return .carrier
        }

        if activeProductIDs.contains(Config.IAP.driverHubProMonthly) {
            return .driver
        }

        if activeProductIDs.contains(Config.IAP.proMonthly) ||
            activeProductIDs.contains(Config.IAP.proAnnual) {
        ...
        case .free:          return nil
        case .driver:        return .driver
        case .ownerOperator: return .ownerOperator
        case .carrier:       return .carrier
        }
    }

    var activePlanRoleName: String {
        entitledAccountRole?.displayName ?? "No active subscription"
    }
    ...
    var hasDriverAccess: Bool {
        #if DEBUG
        if DemoMode.unlockAllFeatures { return true }
        #endif
        if !FeatureFlags.subscriptionsEnabled { return true }
        return activeTier >= .driver
    }

    var hasOwnerOperatorAccess: Bool {
        #if DEBUG
        if DemoMode.unlockAllFeatures { return true }
        #endif
        if !FeatureFlags.subscriptionsEnabled { return true }
        return activeTier >= .ownerOperator
    }
    ...
    func isEntitled(_ feature: Feature) -> Bool {
        ...
        // ASC subscription products are Ready to Submit.
        if !FeatureFlags.subscriptionsEnabled { return true }

        return FeatureAccessManager.hasAccess(currentTier: activeTier, to: feature)
    }

    /// The minimum tier that unlocks a given feature.
    func requiredTier(for feature: Feature) -> SubscriptionTier {
        FeatureAccessManager.requiredTier(for: feature)
    }
}
