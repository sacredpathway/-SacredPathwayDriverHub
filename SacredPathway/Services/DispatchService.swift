...
enum DriverDispatchStatus: String, CaseIterable, Identifiable {
    case available
    case enRoute
    case loaded
    case delivered
...
enum DriverDispatchConnectionAction: String {
    case block = "Block"
    case report = "Report"
    case leave = "Leave Connection"
}

/// Driver Hub's production-safe dispatcher boundary.
///
/// This client intentionally owns only driver-side state. It does not fetch
/// dispatcher dashboards, directories, invoices, fee records, payment records,
/// or management objects. The future separate Dispatcher App should get its
/// own service layer for dispatcher-owned workflows.
@MainActor
final class DriverDispatchClient: ObservableObject {
    static let shared = DriverDispatchClient()

    @Published private(set) var conversations: [DispatchThread] = []
    @Published private(set) var offers: [DispatchLoadOffer] = []
    @Published private(set) var lastMessage: String?
    @Published private(set) var selectedStatus: DriverDispatchStatus = .available

    private init() {}

    var hasConnection: Bool {
        !conversations.isEmpty || !offers.isEmpty
    }
    ...
    func reloadPlaceholders() {
        // No backend reads until the driver/dispatcher RLS migration is approved.
        conversations = []
        offers = []
    }

    func acceptPlaceholderOffer() {
        lastMessage = "Load offer acceptance will activate after the dispatcher backend connection is enabled."
    }

    func declinePlaceholderOffer() {
        lastMessage = "Load offer decline will activate after the dispatcher backend connection is enabled."
    }

    func setStatus(_ status: DriverDispatchStatus) {
        selectedStatus = status
        lastMessage = "Status set to \(status.displayName). Backend sync will activate after the dispatcher connection tables are approved."
    }

    func connectionAction(_ action: DriverDispatchConnectionAction) {
        lastMessage = "\(action.rawValue) will activate after connection moderation is backed by RLS."
    }
}
