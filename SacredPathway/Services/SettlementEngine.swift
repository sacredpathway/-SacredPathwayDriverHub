...
class SettlementEngine {
    #if DEBUG
    private static var didRunCustomFeeRegressionCheck = false
    #endif
    ...
    static func calculate(
        ...
        customDeductions: [SettlementCustomDeduction]? = nil
    ) -> SettlementCalculation {
        #if DEBUG
        if !didRunCustomFeeRegressionCheck {
            didRunCustomFeeRegressionCheck = true
            CustomFeeService.shared.runCustomFeeRegressionCheck()
        }
        #endif
        ...
        let appliedCustomDeductions = customDeductions
            ?? CustomFeeService.shared.activeCustomDeductions(grossPay: totalRevenue)
        let customDeductionsTotal = appliedCustomDeductions.reduce(0) { $0 + $1.amount }
        ...
        return SettlementCalculation(
            ...
            customDeductions: appliedCustomDeductions,
