...
//
//  Window date rule (see `windowDate(for:)`): a load is bucketed by
//  pickupDate, falling back to deliveryDate, then createdAt — so a load with
//  a nil pickupDate never silently vanishes from windowed totals. (This
//  header previously claimed nil-pickup loads were excluded; the code moved
//  to the fallback chain on 2026-06-14 and the comment is corrected as of
//  2026-07-04.) `allTime` counts every load regardless of dates.
// =============================================================================
...
enum WeeklyStatsService {
    ...
    /// through one rule.
    ///
    /// ⚠️ Prefer `contains(period:load:now:)` for whole loads — this
    /// date-only variant has NO fallback for a nil pickup date, so a list
    /// filtered with it can disagree with the revenue number produced by
    /// `revenue(in:loads:)` (which falls back to deliveryDate/createdAt via
    /// `windowDate(for:)`).
    ...
    /// Load-aware membership test using the SAME `windowDate(for:)` fallback
    /// chain (pickup → delivery → created) as the revenue pipeline. Use this
    /// for list filtering so what the user SEES always matches what the
    /// headline number COUNTS. Added 2026-07-04 (Phase 1 financial accuracy).
    static func contains(
        period: StatsPeriod,
        load: Load,
        now: Date = Date()
        ...
        if period == .allTime { return true }
        guard let d = windowDate(for: load) else { return false }
        let i = interval(for: period, now: now)
        return d >= i.start && d < i.end
    }
    ...
    /// The date used to place a load in a time window.
    ///
    /// PICKUP FIRST (changed 2026-06-14): a load is counted in the week it is
    /// PICKED UP, then deliveryDate, then createdAt as a last resort. This makes
    /// the revenue window match the loads-list window (which buckets by
    /// pickupDate via PayWeekService.pickupFalls). Previously revenue bucketed by
    /// deliveryDate, so a load picked up this week but delivering next week was
    /// shown in the "This Week" list yet EXCLUDED from "This Week" revenue — the
    /// exact bug a new load "not showing in This Week revenue" exhibited.
    static func windowDate(for load: Load) -> Date? {
        load.pickupDate ?? load.deliveryDate ?? load.createdAt
    }
    ...
    private static func bucket(
        ...
        let result = deduped.filter {
            guard let d = windowDate(for: $0) else { return false }
            return d >= i.start && d < i.end
        }
        #if DEBUG
        if period == .week {
            func dbg(_ d: Date?) -> String {
                guard let d else { return "nil" }
                let f = DateFormatter(); f.dateFormat = "MM/dd"; f.timeZone = .current
                return f.string(from: d)
            }
            print("[WeeklyRev] window \(dbg(i.start))→\(dbg(i.end))  raw=\(loads.count) deduped=\(deduped.count) counted=\(result.count)")
            for l in deduped {
                let wd = windowDate(for: l)
                let inWeek = wd.map { $0 >= i.start && $0 < i.end } ?? false
                print("[WeeklyRev]  \(inWeek ? "✓COUNTED " : "✗EXCLUDED")"
                    + " id=\(l.id?.uuidString.prefix(8) ?? "—")"
                    + " num=\(l.loadNumber ?? "—")"
                    + " rev=\(l.totalRevenue.map { String(format: "%.0f", $0) } ?? "nil")"
                    + " pickup=\(dbg(l.pickupDate)) delivery=\(dbg(l.deliveryDate)) created=\(dbg(l.createdAt))"
                    + " window=\(dbg(wd)) status=\(l.status ?? "—")")
            }
        }
        #endif
        return result
