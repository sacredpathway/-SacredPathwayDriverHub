...
struct FilePickerView: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, UIDocumentPickerDelegate {
        ...
        func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
            ...
            // PDF → render all pages for OCR, use first page for preview,
            // keep original bytes for downstream storage.
            if ext == "pdf" {
                let pdfImages = Self.renderPDFPages(url: url)
                if let pdfImage = pdfImages.first {
        ...
        private static func renderPDFPages(url: URL) -> [UIImage] {
            guard let document = CGPDFDocument(url as CFURL) else { return [] }

            return (1...document.numberOfPages).compactMap { pageIndex in
