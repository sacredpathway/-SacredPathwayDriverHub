...
final class SettlementCloudStore {
    ...
    func fetchLedger() async throws -> SettlementLedger {
        ...
        do {
            async let settlements: [Settlement] = rows("settlements", order: "created_at")
            async let lines: [SettlementLoadLine] = rows("settlement_loads", order: "sort_order")
            async let adds: [SettlementAddition] = rows("settlement_additions", order: "sort_order")
            async let deds: [SettlementDeduction] = rows("settlement_deductions", order: "sort_order")
            async let recurring: [RecurringDeduction] = rows("recurring_deductions", order: "created_at")
            async let advances: [DriverAdvance] = rows("driver_advances", order: "date")
            async let repayments: [DriverAdvanceRepayment] = rows("driver_advance_repayments", order: "date")
            async let audit: [SettlementAuditEvent] = rows("settlement_audit_events", order: "timestamp")

            var ledger = SettlementLedger(
                settlements: try await settlements,
                loadLines: try await lines,
                additions: try await adds,
                deductions: try await deds,
                recurringDeductions: try await recurring,
                advances: try await advances,
                advanceRepayments: try await repayments,
    ...
    func fetchDriverLedger(linkedDriverIds: Set<UUID>) async throws -> SettlementLedger {
        ...
        do {
            ...
            async let lines: [SettlementLoadLine] = rows("settlement_loads", order: "sort_order", settlementIds: sids)
            async let adds: [SettlementAddition] = rows("settlement_additions", order: "sort_order", settlementIds: sids)
            async let deds: [SettlementDeduction] = rows("settlement_deductions", order: "sort_order", settlementIds: sids)
