...
final class LoadsSyncService: ObservableObject {
    ...
    nonisolated(unsafe) private var loadsDidChangeObserver: NSObjectProtocol?
    ...
    private init() {
        ...
        LocalLoadsRepository.shared.$loads
            .sink { [weak self] _ in
                Task { [weak self] in
                    self?.handleLoadsChangedFromNotification()
                }
            }
        ...
        loadsDidChangeObserver = NotificationCenter.default.addObserver(
            ...
            let changedLoad = notification.object as? Load
            MainActor.assumeIsolated {
                if AppMode.shared.isCloud, let changedLoad {
                    self?.upsertCloudCache(changedLoad)
                } else {
    ...
    // MARK: - Recompute coalescing (Phase 1 · Task 4, 2026-07-04)
    //
    // One user mutation can arrive through MULTIPLE observers in the same
    // runloop tick (LocalLoadsRepository.$loads sink fires on the repo
    // write AND the caller posts `.loadsDidChange`). Each uncoalesced
    // arrival ran dedupe (O(n)) + sort (O(n log n)) + a whole-array
    // `loads` publish — and every publish forces a fresh SwiftUI diff/
    // render pass on the Dashboard, Loads list, and any other observer.
    // Coalescing collapses same-tick storms into a single recompute.
    // Direct API paths (refresh/tombstone/upsert) still recompute
    // immediately — user-initiated actions stay snappy.

    /// True while a coalesced recompute is queued for the next tick.
    private var recomputeScheduled = false

    /// Queue exactly one recompute per MainActor tick, no matter how many
    /// observers fire in that tick.
