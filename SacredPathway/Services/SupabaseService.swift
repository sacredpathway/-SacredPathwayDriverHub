...
class SupabaseService: ObservableObject {
    ...
    func createLoad(_ load: Load) async throws -> Load {
        var load = load
        // Mirror LocalLoadsRepository: a nil pickup would hide a newly created
        // load from current-week dashboards/lists (weekly buckets exclude nil).
        // Default a missing pickup to today; never overwrite a real one.
        if load.pickupDate == nil {
            load.pickupDate = Calendar.current.startOfDay(for: Date())
        }
        #if DEBUG
        print("""
        [createLoad] START
    ...
    func deleteLoad(id loadId: UUID) async throws {
        // Tombstone FIRST so the canonical income list (Dashboard Revenue /
        // Net Profit) drops this load immediately. Cloud Mode's
        // `.loadsDidChange` carries no row object, so a plain recompute would
        // re-read the stale cloudCache and keep counting the deleted load until
        // the next full server refetch (app restart) — that was the "revenue
        // counter doesn't update" bug. tombstone() recomputes right away; the
        // next refresh() wipes the tombstone once the server confirms deletion,
        // and if the delete below throws that same refresh() restores the row.
    ...
    /// Upload a receipt IMAGE (JPEG) to the `documents` bucket + create a
    /// vault row, mirroring `saveDocumentRecord` which is PDF-specific.
    /// Used by the expense form's best-effort cloud copy (Phase 1 · Task 3):
    /// the authoritative receipt lives in ReceiptImageStore on-device; this
    /// makes it visible in the Document Vault for cloud accounts.
    @discardableResult
    func saveReceiptImageRecord(
        jpegData: Data,
        documentType: String,
        title: String,
        loadId: UUID? = nil
        ...
        // Same lowercased-UUID path rule as saveDocumentRecord (Storage RLS
        // compares against lowercase auth.uid()::text).
        let timestamp = Int(Date().timeIntervalSince1970)
        let docId = UUID().uuidString.lowercased()
        let path = "\(userId.uuidString.lowercased())/\(documentType)/\(timestamp)-\(docId).jpg"

        try await uploadDocument(data: jpegData, path: path, contentType: "image/jpeg")
