...
final class SettlementRepository: ObservableObject {
    ...
    /// Read from disk, not the in-memory copy: the in-memory ledger stays
    /// empty until `reload()` has run, and backing up that empty copy would
    /// make a later restore wipe real settlement history. An unreadable file
    /// is left out of the backup (nil), so a restore keeps what is on the
    /// device instead of replacing it. (2.3.3 integration, 2026-09-17.)
    var ledgerForBackup: SettlementLedger? {
        guard isLocal, let store = try? SettlementLocalStore.appDefault() else { return nil }
