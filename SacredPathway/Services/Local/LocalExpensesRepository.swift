...
final class LocalExpensesRepository: ObservableObject {
    ...
    func delete(id: UUID) {
        ...
        // Capture the receipt filename BEFORE removal so the image file is
        // cleaned up with the row (Task 3: no orphan receipt files).
        let receiptFilename = expenses.first(where: { $0.id == id })?.receiptImageFilename
    ...
    func deleteAll(forLoad loadId: UUID) {
        let before = expenses.count
        let receiptFilenames = expenses
            .filter { $0.loadId == loadId }
            .compactMap { $0.receiptImageFilename }
        expenses.removeAll { $0.loadId == loadId }
        if expenses.count != before {
            flush()
            for name in receiptFilenames {
                ReceiptImageStore.shared.delete(filename: name)
            }
        }
    }
