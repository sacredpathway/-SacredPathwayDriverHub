...
// DEBUG-only deterministic launch contract for XCUITest.
// Release builds do not contain the launch mode or its reset behavior.
#if DEBUG
enum DriverHubUITestMode {
    static let isActive = ProcessInfo.processInfo.arguments.contains("-DriverHubUITest")
    static let shouldReset = ProcessInfo.processInfo.environment["DRIVER_HUB_UI_TEST_RESET"] == "1"
    ...
    static func prepare(appMode: AppMode) {
        ...
        if shouldReset {
            if let root = try? LocalStore.rootURL {
                try? FileManager.default.removeItem(at: root)
            }
            UserDefaults.standard.removeObject(forKey: "sp.localstore.lastQuarantine.v1")
        }

        // This selects the same production local-mode path as WelcomeView.
        // It does not bypass field validation or repository behavior.
        appMode.setLocal()
    }
}
#else
...
//   .cloud   → Sign in + subscription flow (AccessGate + Paywall path).
...
final class AppMode: ObservableObject {
    ...
    /// True when the active mode is Cloud Sync.
    ...
    /// User chose "Sign in / Cloud Sync" — called from WelcomeView so the
