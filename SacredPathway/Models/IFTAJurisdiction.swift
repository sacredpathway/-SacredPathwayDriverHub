...
enum IFTAQuarter: Equatable, Hashable {
    ...
    ///
    /// FIX (2026-07-04, Phase 1 financial accuracy): `dateRange.end` is
    /// MIDNIGHT of the quarter's last day (e.g. Mar 31 00:00), so the old
    /// `date <= range.end` comparison silently excluded any entry logged
    /// later on the last day of a quarter (Mar 31 2:00 PM ∉ Q1 and ∉ Q2 —
    /// it vanished from every report). Compare against the exclusive start
    /// of the NEXT day instead, so the full final day counts.
    func contains(_ date: Date) -> Bool {
        let range = dateRange
        guard date >= range.start else { return false }
        let cal = Calendar.current
        let endExclusive = cal.date(byAdding: .day, value: 1, to: cal.startOfDay(for: range.end))
            ?? range.end
...
//
// RATE OVERLAY (2026-07-04, Phase 1 financial accuracy): the static list
// above intentionally carries `taxRatePerGallon: 0.0` — 0 is a SENTINEL for
// "no official rate loaded", never a real rate. When IFTARatesService has a
// validated quarterly matrix (remote or cached), its official rate replaces
// the sentinel here, so every consumer (IFTACalculator, report view, CSV/PDF
// exports) picks up real rates through this one function. When no matrix is
// loaded, callers MUST check `IFTARatesService.shared.ratesAvailable` before
// rendering tax-owed figures — IFTAReportView hides them and shows a banner.
func jurisdictionForCode(_ code: String) -> IFTAJurisdiction? {
    guard let base = iftaJurisdictions.first(where: { $0.code == code.uppercased() }) else {
        return nil
    }
    if let official = IFTARatesService.shared.rate(for: base.code) {
        return IFTAJurisdiction(
            id: base.id,
            code: base.code,
            name: base.name,
            taxRatePerGallon: official
        )
    }
    return base
}
