...
enum SPDate {
    ...
    static var isoFractional: ISO8601DateFormatter {
        let f = ISO8601DateFormatter()
...
struct ExpenseFormPrefill {
    ...
    /// The captured receipt photo from the Smart Scan flow. Previously the
    /// image was OCR'd and then DISCARDED — now it rides the prefill into
...
struct Expense: Codable, Identifiable {
    ...
    /// Filename of the receipt image in ReceiptImageStore
    /// (`Documents/DriverHub/Receipts/<filename>`). FILENAME ONLY — never an
    /// absolute path (container paths change across reinstalls). nil = no
    /// receipt attached. Added 2026-07-04 (Phase 1 · Task 3). Encoded with
    /// `encodeIfPresent`, so expenses without receipts produce no
    /// `receipt_image_filename` key and stay compatible with a cloud schema
    /// that hasn't run the matching migration yet
    /// (supabase/migrations/20260704200000_expenses_receipt_image.sql).
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case receiptImageFilename = "receipt_image_filename"
    ...
    init(from decoder: Decoder) throws {
        ...
        receiptImageFilename = try c.decodeIfPresent(String.self, forKey: .receiptImageFilename)
    ...
    func encode(to encoder: Encoder) throws {
        ...
        // encodeIfPresent: nil receipts add NO key → cloud rows keep saving
        // even before the receipt_image_filename migration is applied.
        try c.encodeIfPresent(receiptImageFilename, forKey: .receiptImageFilename)
